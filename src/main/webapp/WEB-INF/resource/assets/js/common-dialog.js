jQuery.extend(jQuery, {
  // jQuery UI alert弹出提示
  jqalert: function(text) {
	var tipArray = text.split("<br>");
//    var html =
...
//    '</div>';
    ...
    for(var i = 0; i < tipArray.length; i++) {
    	if("" != tipArray[i]) {//空提示不显示
    		subTip += '<p><span class="ui-icon ui-icon-circle-check" style="float: left; margin: 0 7px 0 0;"></span>' + tipArray[i] + '</p>'
    	}
    }
    
    var html = '<div class="dialog" id="dialog-message">' + subTip + '</div>';
    ...
    return $(html).dialog({
      ...
      open: function()
      {
    	  $(this).closest(".ui-dialog").before("<div id='alertmask' class='popover-mask-alert'></p>");
    	  $(this).closest(".ui-dialog").addClass("index-alert");
      },
      close : function() {
			$('#alertmask').remove();
			$(this).dialog("close");
  ...
  // jQuery UI alert弹出提示,点击确定触发方法
  jqalertOnok: function(text,fn1) {
		var tipArray = text.split("<br>");
